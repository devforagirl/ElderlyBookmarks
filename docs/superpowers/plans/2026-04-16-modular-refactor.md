...
- [x] **Step 1.2: 迁移 `safeAPI` 到 `src/utils/helpers.js`**
...
- [x] **Step 1.3: 在 `renderer.js` 中临时保留 `safeAPI` 但通过导入调用 (可选，若尚未启用 ESM)**
...
- [x] **Step 2.1: 创建 `src/core/i18n.js`**
...
- [x] **Step 3.1: 创建 `src/services/bookmark.js`**
  封装所有书签操作。
  ```javascript
...
- [x] **Step 3.2: 将 `renderer.js` 中的数据获取逻辑替换为服务调用**
...
- [x] **Step 4.3: 迁移 `renderer.js` 的全局变量到 `state.js`**
  所有 `let allItems = ...` 替换为 `state.allItems`。

- [x] **Step 4.4: 创建 `src/main.js` 入口文件**
...
- [x] **Step 5.1: 创建 `src/ui/renderers.js`**
- [x] **Step 5.2: 创建 `src/ui/components/List.js`**
- [x] **Step 5.3: 在 `src/main.js` 中组合两者**
- [x] **Step 5.4: 验证视图切换与虚拟滚动**
- [x] **Step 5.5: 提交代码**
...
- [x] **Step 6.1: 删除 `renderer.js`**
- [x] **Step 6.2: 删除 `locales.js`**
- [x] **Step 6.3: 最终全量验证**
- [x] **Step 6.4: 提交代码**
